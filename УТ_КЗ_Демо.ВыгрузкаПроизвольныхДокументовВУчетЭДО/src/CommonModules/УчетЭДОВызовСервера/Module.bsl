	...
	Для Каждого ДанныеДокумента Из ОтправляемыеДокументы Цикл
		Для Каждого СтрокаПечати Из ТаблицаПечати Цикл
			ПечатныеФормы = УправлениеПечатью.СформироватьПечатныеФормы(СтрокаПечати.МенеджерПечати, СтрокаПечати.Макет,
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДанныеДокумента.Документ), Новый Структура("ВыводитьУслуги", Ложь));
			Для Каждого ПечатнаяФорма Из ПечатныеФормы.КоллекцияПечатныхФорм Цикл
				...
				ДанныеДляОтправки.Вставить("СтруктураДокументаМетаданные", ПолучитьМетаданныеДокумента(ДанныеДокумента));
...
Функция РезультатПодготовкиОтправкиДокументов(ВыбранныеДокументы, Отбор = Неопределено) Экспорт
	...
	|		И СтатусЭДО.Свойство = &СвойствоСтатусУчетЭДО
	...
	|		И НомерЭДО.Свойство = &СвойствоНомерУчетЭДО
	...
	|		И ИдДокументаЭДО.Свойство = &СвойствоIdУчетЭДО
	|ГДЕ
	|	РеализацияТоваровУслуг.Ссылка В (&Ссылка)";

	Запрос.УстановитьПараметр("Ссылка", ВыбранныеДокументы);
	Запрос.УстановитьПараметр("СвойствоСтатусУчетЭДО", ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Статус_УчетЭДО"));
	Запрос.УстановитьПараметр("СвойствоНомерУчетЭДО", ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Номер_УчетЭДО"));
	Запрос.УстановитьПараметр("СвойствоIdУчетЭДО", ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("id_УчетЭДО"));

	Выборка = Запрос.Выполнить().Выбрать();

	Пока Выборка.Следующий() Цикл

		// Если документ уже отправлен (есть статус)
		Если Не ПустаяСтрока(Выборка.СтатусЭДО) Тогда
			
			Если НЕ РазрешенаПовторнаяОтправка Тогда
				// Обычная отправка - документ уже отправлен
				...
				Продолжить;
			Иначе
				// Повторная отправка (отзыв)
				Если СтатусыДляОтзыва_УчетЭДО().Найти(Выборка.СтатусЭДО) <> Неопределено
					И Выборка.НомерЭДО <> "" И Выборка.ИдентификаторДокумента <> 0 Тогда
					// Документ можно отозвать (статус "Ожидает подписи")
					...
					Продолжить;
				ИначеЕсли Выборка.СтатусЭДО <> "Отозван" Тогда
					// Документ нельзя ни отозвать, ни отправить повторно
					ДанныеДокумента = ДанныеНеотправляемогоДокумента();
					ДанныеДокумента.Документ = Выборка.Документ;
					ДанныеДокумента.Причина	= СтрШаблон(НСтр("ru = 'Документ в статусе ""%1"". Повторно отправить можно только отозванные документы. Для отзыва документ должен быть в статусе ""Ожидает подписи""'"), Выборка.СтатусЭДО);
					Результат.НеотправляемыеДокументы.Добавить(ДанныеДокумента);
					Продолжить;
				КонецЕсли;
				// Если статус = "Отозван" - пропускаем дальше для повторной отправки
			КонецЕсли;
...
Функция ПолучитьМетаданныеДокумента(ДанныеДокумента)

	СтруктураДокументаМетаданные = Новый Структура;

	СтруктураДокументаМетаданные.Вставить("outlet_id", ДанныеДокумента.ИННКонтрагента);
	СтруктураДокументаМетаданные.Вставить("currency", ДанныеДокумента.ВалютаНаименование);
	СтруктураДокументаМетаданные.Вставить("self_bin", ДанныеДокумента.ИННОрганизации);

	СтруктураДокументаМетаданные.Вставить("title", "Реализация ТМЗ и услуг №" + ДанныеДокумента.Номер);
	СтруктураДокументаМетаданные.Вставить("tag_id", "Реализация ТМЗ и услуг");
	СтруктураДокументаМетаданные.Вставить("source_guid", Строка(ДанныеДокумента.Документ.УникальныйИдентификатор()));
	СтруктураДокументаМетаданные.Вставить("doc_date", ДанныеДокумента.Дата);
	СтруктураДокументаМетаданные.Вставить("doc_number", ДанныеДокумента.Номер);

	СтруктураДокументаМетаданные.Вставить("lines", Новый Массив);

	СтруктураДокументаМетаданные.Вставить("period_from", '00010101');
	СтруктураДокументаМетаданные.Вставить("period_to", '00010101');

	СтруктураДокументаМетаданные.Вставить("emails", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДанныеДокумента.АдресЭП));

	Возврат СтруктураДокументаМетаданные;
