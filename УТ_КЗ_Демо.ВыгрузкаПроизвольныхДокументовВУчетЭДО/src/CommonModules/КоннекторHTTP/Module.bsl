...
// Параметры:
//	URL - Строка - URL ресурса, к которому будет отправлен запрос.
//	Данные - Структура, Соответствие из Строка, Строка, ДвоичныеДанные - см. описание ДополнительныеПараметры.Данные.
//	ДополнительныеПараметры - См. НовыеПараметры
//	Сессия - См. СоздатьСессию
//
// Возвращаемое значение:
...
//	* Заголовки - Соответствие из строка - см. описание Сессия.Заголовки.
//	* Аутентификация - Структура - см. описание Сессия.Аутентификация
//	* Прокси - ИнтернетПрокси - см. описание Сессия.Прокси.
//	* ПараметрыЗапроса - Структура, Соответствие из строка - см. описание Сессия.ПараметрыЗапроса.
...
//	* Cookies - Массив из Строка - см. описание Сессия.Cookies.
...
//	* Json - Структура, Соответствие из строка - данные, которые необходимо сериализовать в JSON.
...
//		** ИмяФункцииВосстановления - Строка - определяет имя функции, которая будет вызывается при чтении каждого свойства и должна иметь следующие параметры:
...
//			Возвращается значение -Произвольный - значение, десериализованное из JSON:
...
//		** ИменаСвойствДляОбработкиВосстановления - Массив	из Строка- определяет массив имен свойств JSON, для которых
...
//			- Структура, Соответствие из Строка - поля формы, которые необходимо отправить в запрос:
//				** Ключ - Строка - имя поля.
//				** Значение - Строка - значение поля.
//	* Файлы - Файл - См. НовыйОтправляемыйФайл, Массив Из См. НовыйОтправляемыйФайл - файлы, к отправке
...
//								- Массив из Строка - повторы будут выполняться для конкретных кодов состояний.
//		Значение по умолчанию: Неопределено.
//
Функция НовыеПараметры() Экспорт

	МаксимальноеВремяПовторов = 600;
	...
	Параметры.Вставить("МаксимальноеВремяПовторов", МаксимальноеВремяПовторов);
...
Функция НовыйОтвет() Экспорт
	...
	КодОтветаСетеваяОшибка = 600; // Сетевая ошибка (>500)

	Результат = Новый Структура;
	Результат.Вставить("Метод", "GET");
	Результат.Вставить("URL", "");
	Результат.Вставить("КодСостояния", КодОтветаСетеваяОшибка);
...
//
// Возвращаемое значение:
//	Структура - Новая аутентификация bearer:
...
//
// Возвращаемое значение:
//	Структура - Новая аутентификация AWS4:
...
//	* ИменаСвойствСоЗначениямиДата -	Массив из Строка, Строка - имена свойств JSON,
//		для которых нужно вызывать восстановление даты из строки.
//	* ИмяФункцииВосстановления - Строка - определяет имя функции, которая будет вызывается при чтении каждого свойства и должна иметь следующие параметры:
//			** Свойство - Строка - указывается только при чтении объектов JSON
//			** Значение - Произвольный - значение допустимого для сериализации типа
...
//
// Возвращаемое значение:
//	Соответствие из Строка, Структура - ответ сервера в виде десериализованного значения JSON.
...
// Параметры:
//	Ответ - См. НовыйОтвет
//	ПараметрыЧтенияXML - ПараметрыЧтенияXML - Параметры чтения, которые будут использованы при чтении данных XML. Подробнее см. в синтакс помощнике метод ЧтениеXML.ОткрытьПоток.
...
Функция КакИсключение(Ответ, Знач ТекстДляПользователя = Неопределено) Экспорт
	...
	ПредставлениеКодаСостоянияHTTP = ПредставлениеКодаСостоянияHTTP(Ответ.КодСостояния);
...
//		** Ключ - Строка - ключ параметра в URL.
//		** Значение - Строка - значение параметра URL;
//						- Массив из Строка - значения параметра (key=value1&key=value2).
...
//	* ИмяФункцииПреобразования - Строка - функция, которая вызывается для всех свойств, тип которых не поддерживает автоматическую сериализацию в JSON.
...
//			** ДополнительныеПараметры - Произвольный - в этом параметре будет передан ДополнительныеПараметрыФункцииПреобразования.
...
Функция СформироватьНовыйURLПриПеренаправлении(Ответ)
	...
	КоличествоДвойныхСлэшей = 2;
	СтруктураURL = РазобратьURL(НовыйURL);
	Если РазбитьСтрокуПоСтроке(НовыйURL, "://").Количество() < КоличествоДвойныхСлэшей Тогда
...
Функция НастройкиПодключения(Сессия, Метод, URL, ДополнительныеПараметры)
	...
	МаксимальноеВремяПовторовПоУмолчанию = 600;
	...
	МаксимальноеВремяПовторов = ЗначениеПоКлючу(ДополнительныеПараметры, "МаксимальноеВремяПовторов", МаксимальноеВремяПовторовПоУмолчанию);
...
Функция ПодготовитьЗаголовокDigest(Сессия, ПодготовленныйЗапрос)
	...
	ЗначениеNonce = Лев(СтрЗаменить(НРег(Новый УникальныйИдентификатор), "-", ""), ЗнаковNonce);
...
Функция КонструкторCookie(Наименование = "", Значение = Неопределено)
	...
	НовыйCookie.Вставить("СрокДействия", СрокДействия);
