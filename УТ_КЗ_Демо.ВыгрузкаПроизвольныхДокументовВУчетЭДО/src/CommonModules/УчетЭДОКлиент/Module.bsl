...
Процедура ПриОкончанииПодписания(Результат, ДополнительныеПараметры) Экспорт
	...
	ПараметрыОтправки = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ДополнительныеПараметры);
	ПараметрыОтправки.Удалить("ОповещениеПоЗавершении");
	
	МассивОтветов = УчетЭДОВызовСервера.ОтправитьПроизвольныеДокументы(Результат.МассивДокументов, ПараметрыОтправки);
